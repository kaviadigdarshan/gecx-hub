"""ZIP artifact assembly for the App Scaffolder and Guardrails Generator accelerators."""
...
from prompts.scaffolder_prompts import build_global_instruction
...
async def build_scaffold_zip(
    ...
    global_instruction = build_global_instruction(
        company=request.use_case.company_name or "our company",
        keywords=request.global_settings.global_instruction_keywords or "professional and helpful",
    )
...
# Canonical public name used by new code
build_app_snapshot_zip = build_scaffold_zip


# ── Guardrail type maps (used by both guardrails_service and guardrails ZIP) ──
...
# ── Guardrails ZIP builder ────────────────────────────────────────────────────

def _guardrails_readme(industry: str, guardrail_count: int) -> str:
...
async def package_guardrails_zip(guardrails: list[dict], industry: str) -> bytes:
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr(
            ...
            _guardrails_readme(industry, len(guardrails)),
        )
    buffer.seek(0)
    return buffer.read()
...
# Canonical public name used by new code
build_guardrails_zip = package_guardrails_zip
