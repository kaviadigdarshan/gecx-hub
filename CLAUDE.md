...
## Superpowers — DARP v3 Policy
Active skills: systematic-debugging, requesting-code-review, subagent-driven-development
Suppressed: brainstorming, writing-plans, using-git-worktrees, finishing-a-development-branch
Reason: v3 execution plan is finalized (23 prompts across 9 sessions). Do not re-plan or create new plans. Execute the existing plan only.

## GECX Hub Core Constraints (do not override)
- CES platform only: ces.googleapis.com — NEVER dialogflow.googleapis.com
- CRM v1 not v3 for project listing
- Gemini JSON: always use json_utils.py + response_mime_type='application/json'
- GCS: USE_LOCAL_STORAGE=true in dev — no signed URLs, no make_public()
- React 18 StrictMode: all auth useEffect guards via useRef
- Zustand persist: scaffoldContext + selectedProject + selectedApp only