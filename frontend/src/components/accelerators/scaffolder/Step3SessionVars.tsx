...
const VAR_TYPES: VariableDeclaration["type"][] = ["STRING", "BOOLEAN", "OBJECT", "ARRAY"]

const DEFAULT_BY_TYPE: Record<VariableDeclaration["type"], VariableDeclaration["defaultValue"]> = {
  STRING: "",
  BOOLEAN: false,
  OBJECT: {},
  ARRAY: [],
...
function DefaultValueInput({
  ...
  if (type === "OBJECT" || type === "ARRAY") {
    const raw = typeof value === "string" ? value : JSON.stringify(value ?? (type === "ARRAY" ? [] : {}), null, 2)
    return (
      <textarea
        rows={2}
        value={raw}
...
function AddVariableForm({ onAdd, onCancel }: { onAdd: (v: VariableDeclaration) => void; onCancel: () => void }) {
  ...
  const [type, setType] = useState<VariableDeclaration["type"]>("STRING")
  const [defaultValue, setDefaultValue] = useState<VariableDeclaration["defaultValue"]>("")
  const [description, setDescription] = useState("")
  const [nameError, setNameError] = useState("")

  const handleTypeChange = (t: VariableDeclaration["type"]) => {
    setType(t)
    ...
    <div className="rounded-lg border border-gecx-200 bg-gecx-50/40 p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select
            value={type}
            onChange={(e) => handleTypeChange(e.target.value as VariableDeclaration["type"])}
